...
for i = 1, 3 do
  Hyper:bind({ "cmd" }, tostring(i), function() addSpaceToScreen(i) end)
end

for i = 1, 9 do
  local str_i = tostring(i)
  Hyper:bind({ "shift" }, str_i, function() removeSpace(i) end)
  Hyper:bind({ "cmd", "ctrl" }, str_i, function() moveWindowToSpace(i) end)
end
